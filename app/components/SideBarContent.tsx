...
import { ContentState, EditorState } from "../interface/types"
...
const SideBarContent = () => {
    ...
    const { name, username, message, backgroundColor } = content;
    ...
    const handelVerified = (satus: any) => {
        dispatch(setIsVerified(satus))
    }
            ...
            <div className="w-full grid grid-cols-4 mt-10 gap-3">
                {
                    patternsComponents.map((Pattern: any, index: number) => {
                        return <div key={index} className={`w-20 h-20 border-4 border-black rounded-md relative cursor-pointer hover:border-8 hover:border-[]`} onClick={() => handelPattern(index + 1)}>
